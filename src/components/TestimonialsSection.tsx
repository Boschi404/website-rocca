          ...
          loop: true
            ...
            <CarouselContent>
              {testimonials.map((testimonial, index) => <CarouselItem key={index} className="md:basis-1/2 lg:basis-1/2 pl-4">
                  <div className="p-6 shadow-md border border-black-100 h-full rounded-xl bg-dental-olive/10">
                    <div className="flex flex-col h-full">
                      <div>
                        <h4 className="font-semibold text-dental-blue mb-2 text-center text-xl">
                          {testimonial.name}
                        </h4>
                        <div className="flex justify-center mb-2">
                          {[...Array(5)].map((_, i) => <svg key={i} className={`w-5 h-5 ${i < testimonial.rating ? 'text-yellow-400' : 'text-gray-300'}`} fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
